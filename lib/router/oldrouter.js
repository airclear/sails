...
var csrfBlueprint = require('../blueprints/csrf');
...
function handler(req, res, next) {
	...
	policyRouter(requestedEntity, actionName, req, res, function afterwards(err) {
		...
		if (actionName === 'index' && !fn) {
			...
			// Use findOne/find instead
			actionName = req.param('id') ? 'findOne' : 'find';
			fn = controller && controller[actionName];
		...
		function afterThat(err, exists, viewPath) {
			...
			// If it's not valid
				...
				// Otherwise, if csrf is enabled and requested, render special csrfPath
				else if(sails.config.csrf && req.route.params.entity === 'csrfToken'){
					return csrfBlueprint(req, res);
				}
